...
func (c *Client) fetchAndCache(cache *pokecache.Cache, url string, result interface{}) error {
...
func (c *Client) GetLocationPokemons(cache *pokecache.Cache, area string) (LocationAreaPokemon, error) {
	url := fmt.Sprintf("%s/location-area/%s", baseURL, area)
	var result LocationAreaPokemon
	err := c.fetchAndCache(cache, url, &result)
	return result, err
}

func (c *Client) GetLocations(cache *pokecache.Cache, pageURL *string) (LocationArea, error) {
